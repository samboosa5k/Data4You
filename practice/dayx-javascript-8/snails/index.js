class Snail {
  ...
  mount( parent ) {
    //  Attach elemen to DOM/execute render
    this.render();
    parent.appendChild( this.snail );
  }

  update() {
    //  For moving the snail
  }
}

document.addEventListener( 'DOMContentLoaded', () => {
  const theParent = document.body;
  const snailBoy1 = new Snail( 'Bob', 1, 'Yellow' );
  snailBoy1.mount( theParent );
  const snailBoy2 = new Snail( 'Pedro', 1, 'Black' );
  snailBoy2.mount( theParent );
  const snailBoy3 = new Snail( 'Snail-name', 1, 'Purple' );
  snailBoy3.mount( theParent );
} )