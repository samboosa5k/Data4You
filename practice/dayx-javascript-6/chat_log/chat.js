...
document.addEventListener( 'DOMContentLoaded', () => {
  ...
  conversation.forEach( ( elem ) => {
    ...
    const position = ( elem.side === 'local' ) ? 'left' : 'right';
    const chatBot = elem.name;
    const chatSegment = `
    
        <div class="chat-segment ${position}">
        <img src="${chatBot}.png" class="chat-avatar"/>
            <div class="chat-text ${position}">
              <h4 class="user-name">${elem.name}</h4>
              <p class="user-message">${elem.text}!</p>
            </div>
        </div>
  
    `;
    chatBody.innerHTML += chatSegment;
  } )
